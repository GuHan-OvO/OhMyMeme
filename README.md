...
- **AI 打标（可选，默认关闭）** — 接入任意 OpenAI 兼容的多模态接口，批量识别表情包内容并生成中文名、画面描述、可见文字、标签、情绪与使用意图；支持按「通用聊天 / 二次元 / 职场 / 游戏群」四种整理风格出词，动图会抽帧拼成宫格一并识别。默认开启审核模式：AI 只产出建议，在设置页或主窗口右键菜单逐条确认后才写入，**AI 标签只做追加，绝不覆盖你已有的标签**；也可关闭审核让 AI 直接入库。总开关关闭时不出现任何入口，也不发起任何网络请求
...
| **AI 智能** | 配置视觉打标与语义检索：总开关（默认关）、接口地址/密钥/模型、整理风格、审核模式、批量与超时参数，以及打标与向量的进度、统计和审核入口（见下） |
...
用视觉大模型给表情包自动打标，把「满屏表情找不到要发的那张」变成可搜索的描述。功能默认关闭，全部配置集中在设置页「AI 智能」分组：

1. **开启** — 勾选「启用 AI 智能」总开关（默认关）。未开启时主窗口右键菜单不出现任何 AI 项，也不会发起任何网络请求
2. **配置打标** — 填写 OpenAI 兼容的接口地址、API Key 与模型名（如 `gpt-4o-mini`、`qwen-vl-plus`），可先点「测试连接」验证连通性
3. **选整理风格** — 「通用聊天 / 二次元 / 职场 / 游戏群」四种预设影响出词取向（同一张图在职场语境下偏「收到、安排」，在游戏群语境下偏「666、上车」）
4. **开始打标** — 设置页「开始打标」批量处理全部待打标表情，或主窗口右键单张表情「AI 打标」；进度弹窗可切到后台运行，也可随时取消。失败项可一键「重试失败项」
5. **审核** — 默认开启审核模式，AI 只产出建议不直接写入。两个入口都能打开同一个审核弹窗：设置页「审核建议」按钮，或主窗口右键表情 →「审核 AI 建议（N）」（仅在有待审建议时出现，N 为待审数量）。逐条查看画面描述、可见文字、标签、情绪与使用意图，标签可自行增删改，确认后「应用此条」或「全部应用」。主窗口卡片上的「AI 待审」角标用于提示哪些表情有待审建议，打标结束或应用建议后角标会自动刷新。**AI 标签只做追加合并，绝不会覆盖你已有的标签**——弹窗里会把你原有的标签单独只读列出
6. **语义检索（可选，推荐开启）** — 独立于打标的第二个开关。开启后为表情生成语义向量，之后在主窗口搜索框**直接输入一句话**即可按含义搜索（如「想怼人的表情」「开心庆祝的」），不必精确匹配名称或标签。配好嵌入接口后点「生成向量」；重复执行不会重复消耗额度（源文本未变则跳过）。AI 打标产出的名称、描述与标签本身就是优质语义信号，故**推荐与打标一起开启**；两者互不依赖，也可只开其一（如已有大量人工标签、只想开语义搜索）。未开嵌入或库中尚无向量时，搜索框自动回退为原有关键词搜索。检索只返回相似度达标的候选：**「检索返回条数」是上限，「最低相似度」（默认 0.35）是过滤线**，低于线的候选视为不相关直接丢弃——两者都在设置页可调。注意不同嵌入模型的分数分布差异很大（本机实测该模型把分数整体压在高位），阈值只作噪声地板，觉得结果太杂可自行上调

> **注意**: 打标会调用第三方接口并产生费用：每批把多张表情（动图抽帧拼成宫格）连同提示词一起发送给你配置的服务商。不确定模型能力时先用少量表情试点。
