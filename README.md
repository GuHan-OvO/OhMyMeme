...
- **批量操作** — 多选模式下支持批量加入分组/移动到分组/tag/删除；批量 tag 为合并追加，不清空各表情已有标签
...
- **右键菜单** — 重命名/收藏/tag/添加分组/加入分组（任意视图可用，子菜单列出全部分组树，可搜索）/从分组移除/删除；表情卡悬停显示快捷收藏按钮，侧边栏分组悬停显示「⋯」快速操作入口
- **设置界面** — 表单改动未保存时关闭会提示确认；局域网开关/密钥传输/存储位置等立即生效的选项带「立即生效」标注
- **标签** — 右键表情打开标签编辑器：点选已有标签、搜索过滤、或输入新建，点击标签按多标签交集筛选；搜索框关键词同时匹配文件名与标签名
- **AI tagging（可选，默认关闭）** — 接入任意 OpenAI 兼容的多模态接口，批量识别表情包内容并生成中文名、画面描述、可见文字、标签、情绪与使用意图；支持按「通用聊天 / 二次元 / 职场 / 游戏群」四种整理风格出词，动图会抽帧拼成宫格一并识别。默认开启审核模式：AI 只产出建议，在设置页或主窗口右键菜单逐条确认后才写入，**AI 标签只做追加，绝不覆盖你已有的标签**；也可关闭审核让 AI 直接入库。总开关关闭时不出现任何入口，也不发起任何网络请求
...
| **微信导入** | 从微信电脑版缓存提取收藏表情（Windows，需微信运行）：随包内置辅助二进制提取密钥 → 解密数据库 → CDN 下载校验；支持多账号选择；执行前校验二进制 SHA-256，不匹配拒绝执行 |
| **AI** | 配置视觉 tagging 与语义检索：总开关（默认关）、接口地址/密钥/模型、整理风格、审核模式、批量与超时参数，以及 tagging 与向量的进度、统计和审核入口（见下） |
...
用视觉大模型给表情包自动 tagging，把「满屏表情找不到要发的那张」变成可搜索的描述。功能默认关闭，全部配置集中在设置页「AI」分组：

1. **开启** — 勾选「启用 AI」总开关（默认关）。未开启时主窗口右键菜单不出现任何 AI 项，也不会发起任何网络请求
2. **配置 tagging** — 填写 OpenAI 兼容的接口地址、API Key 与模型名（如 `gpt-4o-mini`、`qwen-vl-plus`），可先点「测试连接」验证连通性
3. **选整理风格** — 「通用聊天 / 二次元 / 职场 / 游戏群」四种预设影响出词取向（同一张图在职场语境下偏「收到、安排」，在游戏群语境下偏「666、上车」）
4. **开始 tagging** — 设置页「开始 tagging」批量处理全部待 tagging 表情，或主窗口右键单张表情「AI tagging」；进度弹窗可切到后台运行，也可随时取消。失败项可一键「重试失败项」
5. **审核** — 默认开启审核模式，AI 只产出建议不直接写入。两个入口都能打开同一个审核弹窗：设置页「审核建议」按钮，或主窗口右键表情 →「审核 AI 建议（N）」（仅在有待审建议时出现，N 为待审数量）。逐条查看画面描述、可见文字、标签、情绪与使用意图，标签可自行增删改，确认后「应用此条」或「全部应用」。主窗口卡片上的「AI 待审」角标用于提示哪些表情有待审建议，tagging 结束或应用建议后角标会自动刷新。**AI 标签只做追加合并，绝不会覆盖你已有的标签**——弹窗里会把你原有的标签单独只读列出
6. **语义检索（可选，推荐开启）** — 独立于 tagging 的第二个开关。开启后为表情生成语义向量，之后在主窗口搜索框**直接输入一句话**即可按含义搜索（如「想怼人的表情」「开心庆祝的」），不必精确匹配名称或标签。配好嵌入接口后点「生成向量」；重复执行不会重复消耗额度（源文本未变则跳过）。AI tagging 产出的名称、描述与标签本身就是优质语义信号，故**推荐与 tagging 一起开启**；两者互不依赖，也可只开其一（如已有大量人工标签、只想开语义搜索）。未开嵌入或库中尚无向量时，搜索框自动回退为原有关键词搜索。检索只返回相似度达标的候选：**「检索返回条数」是上限，「最低相似度」（默认 0.35）是过滤线**，低于线的候选视为不相关直接丢弃——两者都在设置页可调。注意不同嵌入模型的分数分布差异很大（本机实测该模型把分数整体压在高位），阈值只作噪声地板，觉得结果太杂可自行上调

> **注意**: tagging 会调用第三方接口并产生费用：每批把多张表情（动图抽帧拼成宫格）连同提示词一起发送给你配置的服务商。不确定模型能力时先用少量表情试点。
